...
这是一个适用于 Mihon 及其分支，用于阅读**哔哩轻小说**的插件，使用 `TextInterceptor` 将文本转成图片显示
...
### [1.5.16] - 2026-03-31
- **将插件 API 级别从 1.4 升到 1.5**
- 添加深度链接功能，允许用户通过源站小说的URL直接跳转到Mihon内的详细页。如果提供的URL无效，则会自动跳转到全局搜索。有两种触发方式：
  1. 在本插件的系统设置页中，设置应用内允许打开的链接，勾选全部后即可通过直接点击URL跳转到应用内详细页（注：该流程仅适用于类原生系统，其它系统请自行研究）
  2. 直接将URL分享到 `Mihon(搜索)` 即可跳转
